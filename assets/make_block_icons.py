...
def draw_pin(d, c):
    """항상 위 — **압정 정면** (사용자 결정 2026-07-30).

    예전 그림(넓은 머리 + 가는 바늘)은 깔때기처럼 보인다는 지적을 받았다
    (사용자 지적 2026-07-30: "항상 위 아이콘이 어색합니다"). 머리를 동그라미
    하나로 바꾸고 그것을 포인트색으로 채운다 — 획이 둘뿐이라 16px 에서도
    안 뭉개지고, '고정'이라는 관습이 바로 읽힌다.
    """
    cx = N / 2
    r = N * 0.185
    hy = N * 0.34
    d.ellipse([cx - r, hy - r, cx + r, hy + r], fill=ACCENT + (255,))
    _stroke(d, c, 0.50, (hy + r) / N, 0.50, 0.87)
