...
def draw_settings(d, c):
    ...
    이 여덟 개를 **채워** 붙였더니 이 한 벌에서 혼자 시커먼 덩어리였다.
    고리 하나에 짧은 획 여덟 개로 바꾼다 — 멀리서는 톱니, 가까이서는 선이다.
    """
    cx = cy = N / 2
    ri, ro = N * 0.30, N * 0.415
    d.ellipse([cx - ri, cy - ri, cx + ri, cy + ri], outline=c, width=W)
    r2 = N * 0.115
    d.ellipse([cx - r2, cy - r2, cx + r2, cy + r2], outline=c, width=W)
    for k in range(8):
...
def draw_help(d, c):
    """도움말 — 동그라미 안 물음표. 글자를 쓰지만 이건 만국 공용 기호다."""
    d.ellipse(_p(0.13, 0.13, 0.87, 0.87), outline=c, width=W)
    _text_mid(d, c, "?", N // 2, int(N * 0.47), int(N * 0.50), bold=False)


def draw_undo(d, c):
    """되돌리기 — 왼쪽으로 도는 화살표.

    원을 한 바퀴 가깝게 그렸더니 16px 에서 **그냥 동그라미**가 됐다 (실측
    2026-07-29). 호를 짧게 자르고 그 자리를 화살촉에 준다.
    """
    d.arc(_p(0.15, 0.20, 0.85, 0.90), start=185, end=55, fill=c, width=W)
    _chevron(d, c, 0.15, 0.55, 0.115, dx=0, dy=1)


def draw_pin(d, c):
    """항상 위 — 압정. ⇧(위 화살표)는 '올린다'로 읽혀 상태가 안 보인다.

    머리와 몸통의 폭 차이가 작으면 16px 에서 **콘센트 플러그**로 읽힌다
    (실측 2026-07-29). 머리를 넓게, 바늘을 가늘고 짧게.
    """
    _stroke(d, c, 0.17, 0.15, 0.83, 0.15)
    _stroke(d, c, 0.32, 0.15, 0.36, 0.60, 0.64, 0.60, 0.68, 0.15)
    _stroke(d, c, 0.50, 0.60, 0.50, 0.87)
