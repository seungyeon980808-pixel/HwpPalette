...
from hwp_palette.core import appinfo                     # 이름·버전·슬로건 한 곳 모음
...
from hwp_palette.ui import library_ui
from hwp_palette.model import palette
from hwp_palette.ui import palette_ui
from hwp_palette.model import builtin_actions               # 팔레트에 놓는 '도구' 블럭 카탈로그
from hwp_palette.core import hotkey                        # 한글에서도 먹는 전역 단축키
from hwp_palette.design import ui_fx                         # 호버 보간·누름 피드백 (애플 A안)
from hwp_palette.core import screens                       # 여러 모니터를 합친 좌표
from hwp_palette.ui import help_ui                       # 도움말 창 (기능별 사전)
...
# __file__ 을 쓰지 않는다 — 이 파일은 hwp_palette/app.py 라 "python app.py"
# 라고 찍히는데, 실제로 켜는 방법은 뿌리의 main.py 다 (2026-07-28 폴더 개편).
...
# 2026-07-30: 공통 팔레트를 **아이콘 주도**로 바꾸면서 기호를 키웠다
# (사용자 결정, docs/mockups/design-2026-07-picked.html). 이름은 줄이지
# 않는다 — "지금보다 25% 확대"(2026-07-25) 결정과 어긋나기 때문.
# 개인 팔레트에는 아이콘을 아예 안 붙인다 (_render_block_grid).
#
# 2026-07-31: 16/13 이 실제 화면에서 이름을 누르는 무게였다(사용자 지적:
# "지금의 사이즈보다 25% 줄이도록") — 25% 내려 12/10. 설정 미리보기
# (palette_ui._make_tile 의 기준값 12/10)와 짝이므로 함께 바꿔야 한다.
_BLOCK_ICON_FS = 12
_BLOCK_ICON_FS_2LINE = 10
...
# ══════════════════════════════════════════════════════
# 한글과 도킹 — 우리 창이 한글을 감싼다 (2026-07-29)
#
# 창 둘은 끝까지 남남이다 (SetParent 를 쓰지 않는 이유는 docs/EMBED_검토.md).
...
def _enter_dock(hwnd):
    r"""한글을 우리 창 안에 **감싼다** (2026-07-29 재작업).

    첫 판은 우리 창을 얇은 세로 띠로 만들어 한글 옆에 매 틱 붙였는데,
    사용자 지적대로 "엄청 버벅거리고 짤리는" 화면이 됐다 — 창 둘이 각자
    움직이며 서로를 쫓는 것이 그대로 보였기 때문이다.

    이제는 템플릿을 고칠 때와 같은 방식이다: 우리 창은 가만히 있고
    **한글이 우리 판 자리로 들어온다** (hwp_dock.Dock). 창을 끌면 한글이
    ...
    if spot:
        root.geometry(f"{_DOCK_W}x{_DOCK_H}+{spot[0]}+{spot[1]}")
    else:
        root.geometry(f"{_DOCK_W}x{_DOCK_H}")
    root.update_idletasks()
...
def _exit_dock():
    """감싸기를 풀고 평소 창으로 돌아온다. 한글은 도킹 전 자리로."""
...
def _restore_normal_layout():
    ...
    for key in ("bar", "host"):
        if _dock[key] is not None:
            _dock[key].destroy()
            _dock[key] = None
    # 도킹 중에 새로 붙인 구분선까지 걷어낸다 — 남으면 평소 화면에 줄이 는다
    for w in root.pack_slaves():
        if w is not misc_row and all(w is not p for p, _i in (_dock["packs"] or [])):
            w.destroy()
    for w, info in (_dock["packs"] or []):
        if w is not misc_row:
            w.pack(**info)
    _dock["packs"] = None
    # 크기를 **먼저** 되돌리고 그 다음에 고정한다 (실측 2026-07-29).
    # resizable(False, False) 는 그 순간의 크기를 최소·최대값으로 박아 버려서,
    # 먼저 부르면 뒤이은 geometry 가 먹지 않고 창이 감싸기 크기(1180x900)로
    # 남는다 — 도킹을 뗐는데 창만 거대한 채였다.
    if _dock["geo"]:
        root.geometry(_dock["geo"])
        _dock["geo"] = None
    root.update_idletasks()
    root.resizable(False, False)
