...
# ── 칸 오른쪽 세로 띠 (2026-08-01, 피드백 037 · 안 B) ──
# '무엇인가'를 말하는 표시라 상태 색(파랑·초록·코랄)과 안 겹치는 계열을 쓴다.
# 겹치기(택일)와 섞기(합체)는 다른 물건이라 **글자도 색도 가른다** —
# 섞기 = 보라 MIX 띠, 겹치기 = 청록 계열에 **개수 숫자**만.
MIX_BG, MIX_FG = "#f3eefc", "#6639ba"          # 꾸러미(섞은 물감)
STACK_BG, STACK_FG = "#e6f4f1", "#0e7490"      # 겹친 칸 (개수 표시)
...
BLOCK_ICON = {
    ...
    # ('기본 서식' 도구는 2026-07-31 폐지 — 그 자리에 쓰던 "가a" 는 아래
    #  TYPE_ICON["function"] 이 그대로 물려받는다)
    #
    # 자간 맞춤 — 글자 사이를 좁히는 일이라 좌우에서 안으로 미는 기호를 쓴다.
    "spacing_fit":  "→←",
...
TYPE_ICON = {
    "template": "▦",
    "form":     "▤",
    # "가a" = 한/영 글꼴 견본 표기 (윗 BLOCK_ICON 머리말의 그 이유·그 글자)
    "function": "가a",
